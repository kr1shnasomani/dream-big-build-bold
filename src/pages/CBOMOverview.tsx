...
const kpiCards = [
  { label: 'Applications Covered', value: assets.length, color: 'var(--brand-primary)' },
  { label: 'Sites Surveyed', value: assets.filter(a => a.type === 'web').length, color: 'var(--brand-primary)' },
  { label: 'Active Certificates', value: assets.filter(a => a.certInfo.days_remaining > 0).length, color: 'var(--status-safe)' },
  { label: 'Weak Crypto Instances', value: assets.filter(a => a.qScore <= 40).length, color: 'var(--status-critical)' },
  { label: 'Certificate Issues', value: assets.filter(a => a.certInfo.days_remaining <= 30 && a.certInfo.days_remaining > 0).length, color: 'var(--status-warn)' },
  { label: 'PQC-Ready (%)', value: `${Math.round((assets.filter(a => a.status === 'elite-pqc' || a.status === 'safe').length / assets.length) * 100)}%`, color: 'var(--status-safe)' },
];
